...
def about_us_display():
    ...
    nisha = tk.Label(A, text='Nisha Yadav')
    kriti = tk.Label(A, text='Kriti')
    lekha = tk.Label(A, text='Lekha K')

    #start_info.grid(row=1, column=1)
    info_.grid(row=3, column=1)
    made_by.grid(row=22, column=1)
    kriti.grid(row=23, column=1)
    lekha.grid(row=24, column=1)
    nisha.grid(row=25, column=1)
    A.mainloop()
